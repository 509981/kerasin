...
**print_code(idx,limit=0,valname='x')**
  - *bot_idx* - индекс бота в популяции
  - *limit* - Вывод первыx limit параметров функции. 0 - все
  - *valname* - Имя переменной для подстановки в код

Получение кода сети на Питон в функуиональной нотации керас. Генерируемый исходный код требует доработки. Ветвления не обрабатываются.
